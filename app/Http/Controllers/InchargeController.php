...
class InchargeController extends Controller
{
    ...
    public function studentStatusAssistant(Request $request,$departmentname ){
        ...
        for ($i=0; $i < count($ids); $i++) { 
            ...
            $lists = Lists::where([['student_id', '=', $ids[$i]], ['department_id', '=', $department->id]])->firstOrFail();
            $lists->status = "Pre-Approved";
            $lists->date_cleared = NULL;
            $lists->save();
    ...
    public function edit_status_assistant($id){
        $student_id = $id;
        $status;
        $department_id;
        $type = Session::get('incharge_id');
        $users = DB::table('departments')
            ...
            ->where([
                ['lists.student_id', '=', $id],
                ['departments.assistant_incharge', '=', $type]
            ])->get();
    ...
    public function edit_notes_assistant(Request $request, $id){
    ...
    public function update_notes_assistant( $departmentname, $id){
    	...
    	return view("Modals.edit_notes_assistant")->with("lists",$lists)->with("show",$show)->with("student", $student);       
